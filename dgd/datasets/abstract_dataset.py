        ...
        for split in ['train', 'val', 'test']:
            for i, data in enumerate(self.dataloaders[split]):
                try:
                    ...
                    unique, counts = torch.unique(data.to_homogeneous().batch, return_counts=True)
                for count in counts:
                    all_counts[count] += 1
    ...
    def edge_counts(self):
        ...
        for split in ['train', 'val', 'test']:
            for i, data in enumerate(self.dataloaders[split]):
                ...
                assert torch.all(edge_types[1:] > 0), edge_types
...
class AbstractDatasetInfos:
    ...
    def compute_input_output_dims(self, datamodule, extra_features, domain_features):
        ...
        ex_extra_feat = extra_features(example_data, example_batch.pos)
