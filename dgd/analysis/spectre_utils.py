###############################################################################
# Adapted from https://github.com/cvignac/DiGress which was
# adapted from https://github.com/lrjconan/GRAN which was
...
from metrics.mmd.calculators import *
...
def degree_stats(graph_ref_list, graph_pred_list, is_parallel=True, compute_emd=False):
    ...
    histogram_from_pred_sample(sample_ref, sample_pred, "Degree")
...
def clustering_stats(graph_ref_list,
    ...
    histogram_from_pred_sample(sample_ref, sample_pred, "Clustering")
...
class SpectreSamplingMetrics(nn.Module):
        ...
        for i, batch in enumerate(loader):
            ...
            for j, data in enumerate(data_list):
                try:
                    ...
                    networkx_graphs.append(to_networkx(data.to_homogeneous(), node_attrs=None, edge_attrs=None, to_undirected=True,
                                                       remove_self_loops=True))
        return networkx_graphs
    ...
    def forward(self, generated_graphs: list, name, current_epoch, val_counter, save_graphs=True, test=False):
        ...
        if 'degree' in self.metrics_list:
            print("Computing degree stats..")
            degree = degree_stats(self.test_graphs[:len(networkx_graphs)], networkx_graphs, is_parallel=True,
                                  compute_emd=self.compute_emd)
        ...
        if 'clustering' in self.metrics_list:
            print("Computing clustering stats...")
            clustering = clustering_stats(self.test_graphs[:len(networkx_graphs)], networkx_graphs, bins=100, is_parallel=True,
                                          compute_emd=self.compute_emd)
        ...
        if 'nodes' in self.metrics_list:
            print('Computing MMD for nodes')
            num_nodes_test = [len(gg.nodes) for gg in self.test_graphs]  # shape B X 1
            num_nodes_gen = [len(aa.nodes) for aa in networkx_graphs]
            mmd_nodes = compute_mmd([np.bincount(num_nodes_test[:len(networkx_graphs)])], [np.bincount(num_nodes_gen)],
                                             kernel=gaussian_tv)

            to_log['nodes'] = mmd_nodes
            wandb.run.summary['nodes'] = mmd_nodes

        if 'radius' in self.metrics_list:
            print("Computing radius stats...")
            radius = radius_stats(self.test_graphs[:len(networkx_graphs)], networkx_graphs, is_parallel=True)
            to_log['radius'] = radius
            wandb.run.summary['radius'] = radius

        if 'community' in self.metrics_list:
            print("Computing community stats...")
            community = community_stats(self.test_graphs[:len(networkx_graphs)], networkx_graphs, is_parallel=True)
            to_log['community'] = community
            wandb.run.summary['community'] = community
...
class InMemorySpectreSamplingMetrics(nn.Module):
    def __init__(self, test_graphs, compute_emd, metrics_list, use_wandb=True):
        super().__init__()
    ...
    def forward(self, generated_graphs: list, name, current_epoch, val_counter, save_graphs=True, test=False):
        ...
        adjacency_matrices = []
        print("Building networkx graphs...")
        if type(generated_graphs[0]) != nx.Graph:
            for graph in generated_graphs:
                node_types, edge_types = graph
                A = edge_types.bool().cpu().numpy()
                adjacency_matrices.append(A)
        ...
        if 'radius' in self.metrics_list:
            print("Computing radius stats...")
            radius = radius_stats(self.test_graphs[:len(networkx_graphs)], networkx_graphs,
                                  compute_emd = self.compute_emd, is_parallel=True)
            to_log['radius'] = radius
            if self.use_wandb:
                wandb.run.summary['radius'] = radius
        ...
        print("MMD Scores:")
        for metric in to_log:
            print(f"{metric}: {to_log[metric]}")
...
class SBMSamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, dataloaders):
        super().__init__(dataloaders=dataloaders,
                         compute_emd=False,
                         metrics_list=['nodes', 'degree', 'clustering', 'radius'])
...
class FBSamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, dataloaders):
        super().__init__(dataloaders=dataloaders,
                         compute_emd=False,
                         metrics_list=['nodes', 'degree', 'clustering', 'radius'])

class ShetlandSamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, dataloaders):
        super().__init__(dataloaders=dataloaders,
                         compute_emd=False,
                         metrics_list=['nodes', 'degree', 'clustering', 'radius'])
...
class FullSampleMetrics(InMemorySpectreSamplingMetrics):
    def __init__(self, test_graphs, use_wandb=True):
        super().__init__(test_graphs = test_graphs,
                         compute_emd=False,
                         metrics_list=['nodes', 'degree', 'clustering', 'radius'],
                         use_wandb=use_wandb)

class PostProcessSampleMetrics(InMemorySpectreSamplingMetrics):
    def __init__(self, test_graphs, use_wandb=True):
        super().__init__(test_graphs = test_graphs,
                         compute_emd=True,
                         metrics_list=['nodes', 'degree', 'clustering', 'radius', 'spectre'],
                         use_wandb=use_wandb)

class ByClassSampleMetrics():
    def __init__(self, real_G, synth_G):
        all_nodes = synth_G.nodes(data=True)
        category_values = [node[1]["target"] for node in all_nodes]
        self.unique_categories = np.unique(category_values).tolist()
        ...
        for category in self.unique_categories:
            nodes = (
                node
                for node, data
                in real_G.nodes(data=True)
                if data.get("target") == category
            )

            subg = real_G.subgraph(nodes)
            self.test_graphs.append(subg)

            nodes = (
                node
                for node, data
                in synth_G.nodes(data=True)
                if data.get("target") == category
            )

            subg = synth_G.subgraph(nodes)
            self.synth_graphs.append(subg)

        self.compute_mmds()


    def compute_mmds(self):

        for i, category in enumerate(self.unique_categories):
            category_synth = [self.synth_graphs[i]]
            category_real  = [self.test_graphs[i]]
            try:
                calculator = FullSampleMetrics(category_real)
                calculator(category_synth, "By Class Sampling Metrics", i, 0)
            except:
                print(f"Couldn't calculate MMDs for subgraph with category {category}")



# class ByClassSampleMetrics(InMemorySpectreSamplingMetrics):
...
#         category_values = [node[1]["target"] for node in all_nodes]
#         unique_categories = np.unique(category_values).tolist()
#
#         test_graphs = [G]
...
#             test_graphs.append(subg)
#
#         super().__init__(test_graphs = test_graphs,
#                          compute_emd=False,
#                          metrics_list=['nodes', 'degree', 'clustering'])
