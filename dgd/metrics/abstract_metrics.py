...
class SumExceptBatchKL(Metric):
    ...
    def update(self, p, q) -> None:
        self.total_value += F.kl_div(q.to(torch.float32), p.to(torch.float32), reduction='sum')
...
class FocalEntropyMetric(Metric):
    def __init__(self):
        super().__init__()
        self.add_state('total_focal', default=torch.tensor(0.), dist_reduce_fx="sum")
        self.add_state('total_samples', default=torch.tensor(0.), dist_reduce_fx="sum")

    def update(self, preds: Tensor, target: Tensor) -> None:
        """ Update state with predictions and target
            preds: Predictions from model, (bs * n, d) or (bs * n * n * d)
            target: Ground truth, (bs * n, d) or (bs * n * n * d)"""

        # target = torch.argmax(target, dim=-1)
        output = focal_loss.sigmoid_focal_loss(preds, target, reduction="sum")

        self.total_focal += output
        self.total_samples += preds.size(0)

    def compute(self):
        return self.total_focal / self.total_samples
...
class KLLMetric(Metric):
    ...
    def update(self, preds: Tensor, target: Tensor) -> None:
        ...
        # target = torch.argmax(target, dim=-1)
        output = self.kl_loss(preds, target)
